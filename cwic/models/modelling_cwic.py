...
class CWICDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: CWICConfig, layer_idx: int):
        ...
        self.self_attn = CWICAttention(config=config, layer_idx=layer_idx)
...
class CWICPreTrainedModel(PreTrainedModel):
    ...
    _no_split_modules = ["CWICDecoderLayer"]
    ...
    @torch.no_grad()
    def use_parameter_cache(self, use=True):
        for m in self.modules():
...
class CWICModel(CWICPreTrainedModel):
    def __init__(self, config: CWICConfig):
        ...
        self.layers = nn.ModuleList(
            [CWICDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.rotary_emb = LlamaRotaryEmbedding(config=config)
        self.gradient_checkpointing = False

        self.embed_proj = nn.Linear(config.hidden_size, config.hidden_size, bias=False)

        # Initialize weights and apply final processing
        self.post_init()

        try:
            self.embed_proj.weight.data = torch.eye(config.hidden_size)
        except:
            logger.warning("Could not initialize embed projection to identity.")
...
@auto_docstring
class CWICForCausalLM(CWICPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: CWICConfig):
        ...
        try:
            ...
            logger.warning("Could not initialize head projection to identity.")
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # TODO: create an output version that includes projected hidden states
        out = CausalLMOutputWithPastAndActiveParameters(
            loss=loss,
            logits=logits,
