...
def _convert_linear(cwic: CWICLinear, linear: nn.Linear):
...
def _convert_mlp(cwic: CWICMLP, mlp: LlamaMLP):
...
def _convert_attn(cwic: CWICAttention, attn: LlamaAttention):
    ...
    _convert_linear(cwic.qkv_proj, llama_qkv)

    _convert_linear(cwic.o_proj, attn.o_proj)


def _convert_layer(cwic: CWICDecoderLayer, layer: LlamaDecoderLayer):

    _convert_norm(cwic.input_layernorm, layer.input_layernorm)
    _convert_attn(cwic.self_attn, layer.self_attn)

    _convert_norm(cwic.post_attention_layernorm, layer.post_attention_layernorm)
    _convert_mlp(cwic.mlp, layer.mlp)
...
def llama_to_cwic(
    ...
    **kwargs,
    ...
    _convert_norm(model.model.norm, llama.model.norm)
    _convert_linear(model.lm_head, llama.lm_head)

    for i in range(config.num_hidden_layers):
        _convert_layer(model.model.layers[i], llama.model.layers[i])
