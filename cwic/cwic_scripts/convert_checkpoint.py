...
def extract_keys(checkpoint, name):
    c = {}
...
def load_linear(module: CWICLinear, checkpoint: dict, name: str, layer: int):
...
def load_mlp(module: CWICMLP, checkpoint: dict, name: str, layer: int):
    ...
    load_linear(module.gate, c, "gate", layer)

    load(module.up.weight, c["up.kernel"][layer])
...
def load_layer(module: CWICDecoderLayer, checkpoint: dict, name: str, layer: int):
    ...
    load(module.input_layernorm.weight, c["attention_norm_i.scale"][layer])
    load_linear(module.self_attn.qkv_proj, c, "attention.wqkv_i", layer)
    load_linear(module.self_attn.o_proj, c, "attention.wo_i", layer)

    load(module.post_attention_layernorm.weight, c["ffn_norm_i.scale"][layer])
    load_mlp(module.mlp, c, "feed_forward.ffn_i", layer)
...
def main():
    ...
    config.head_stripe_size = (
        checkpoint["lm_head_i.W.kernel"].shape[-1] // checkpoint["lm_head_i.thresholds"].shape[-1]
    )
    print(
        f"Config loaded with stripe_size={config.stripe_size} and head_stripe_size={config.head_stripe_size}!"
    )
    ...
    load(
        ...
        F.linear(checkpoint["wte.embedding"], checkpoint["embed_proj_i.kernel"].T),
    )
    load(model.model.norm.weight, checkpoint["ln_f_i.scale"])
    load_linear(model.lm_head, checkpoint, "lm_head_i", None)

    # Decoder layers
    for i, layer in enumerate(model.model.layers):
        load_layer(layer, checkpoint, "h.blocks", i)
